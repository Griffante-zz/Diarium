...
class AvaliacaoController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['avaliacao']))
		{
			...
			$model->turma=$turma->id;
			if ($model->validate()){
				if($model->save()){
					
					$dataProviderLista = new CActiveDataProvider('lista_de_alunos', array(
							'criteria'=>array(
									'condition'=>'turma='.$model->turma0->id,
							),
							'countCriteria'=>array(
									'condition'=>'turma='.$model->turma0->id,
							),
					));
					$dataProviderLista->setPagination(false);
					
					
					foreach ($dataProviderLista->getData() as $aluno){
						$nota=new avaliacao_aluno;
						$nota->avaliacao = $model->id;
						$nota->nota = 0.0;
						$nota->aluno = $aluno->Aluno;
						if($nota->validate()){
							$nota->save();
						}
	...
	public function loadModel()
	{
		...
		{
			if(isset($_GET['id']))
				$this->_model=avaliacao::model()->findbyPk($_GET['id']);
			if($this->_model===null)
